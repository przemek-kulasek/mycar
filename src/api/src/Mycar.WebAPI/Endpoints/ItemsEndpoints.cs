...
public static class ItemsEndpoints
{
    ...
    private static async Task<IResult> GetByOperationId(IMediator mediator,
        [AsParameters] GetItemsByOperationIdQuery query)
    {
    ...
    private static async Task<IResult> Post(IMediator mediator, [AsParameters] CreateItemCommand command)
    {
        ...
        return Results.Created($"/cars/{command.CarId}/operations/{command.OperationId}/items/{result}",
            new { id = result });
    }
}