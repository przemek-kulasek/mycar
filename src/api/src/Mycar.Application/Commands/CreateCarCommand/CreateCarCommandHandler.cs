...
public class CreateCarCommandHandler : IRequestHandler<CreateCarCommand, Guid>
{
    ...
    {
        var carExists = await _mycarContext.Cars.AnyAsync(
            x => x.IdentificationNumber == request.Car.IdentificationNumber || x.Id == request.Car.Id,
            cancellationToken);

        if (carExists) throw new ValidationException("Car already exist.");
        ...
        _logger.LogInformation("Car has been created. VIN: {IdentificationNumber}, Id: {Id}",
            newCar.IdentificationNumber, newCar.Id);
